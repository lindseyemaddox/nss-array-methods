...
console.log("hello, spam");

// You have been tasked with building a list of email addresses of
// all of our customer's contacts so we can spam them with an email 
// about Miffles the Vampire Weiner Dog.

// Create a new project in workspace.
// In your JavaScript file, paste the customers array that contains 
// the emails we need to retrieve.
...
// meaning one iteration inside another one - since you need to iterate 
// the entire array of customers, and then iterate the array of emails 
// for each one.
...
// 1. extract customers' email addresses - customers.contacts.email
// 2. iterate through the entire array of customers - map()
// 3. store customers' emails in a new array using a nested array method 
// 4, iterate the array of emails for each customer - map()

const getEmailAddresses = customers.map(person => person.contacts.email)
  .map((accumulator, counter) => accumulator.concat(counter));
output.innerHTML += getEmailAddresses.join(" <br/>");

console.log("getEmailAddresses: ", getEmailAddresses);
